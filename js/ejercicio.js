...
function fetchCharacters() {
    fetch(`${apiUrl}?limit=${limit}&page=${currentPage}`)
        ...
        .then(data => {
            charactersContainer.innerHTML = ''; // Limpia antes de agregar nuevos personajes
            totalPages = data.meta.totalPages; // Guarda el total de páginas
            data.items.forEach(character => createCharacterCard(character));
            updatePagination(); // Actualiza los botones de paginación
        })
